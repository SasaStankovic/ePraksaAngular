...
export class DnevnikRadaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe((x: any) => {
      this.studentId = x['studentId'] ? x['studentId'] : this.auth.userData.id;
      this.internshipId = x['internshipId'];
      if (this.auth.isStudent()) {
        ...
        this.workDiaryService.getWorkDiaryByStudentAndIntern(this.studentId, this.internshipId).subscribe({
          next: (res: any) => {
            this.help(res);
          },
          error: err => { console.log(err); this.hasWD = false; },
  ...
  submitWorkDiaryId(index: number) {
    ...
    if (!this.getEntryForm(index).value.entryId) {
      ...
      this.workDiaryService.submitInput(this.getEntryForm(index).value).subscribe({
        next: res => {
          let k = this.snackBar.open("Uspješno ste sačuvali izvještaj!", "Ok").onAction().subscribe(() => {
            location.reload();
            k.unsubscribe();
          });
        },
      ...
      this.workDiaryService.editWorkDiaryEntry(tmp, this.workDiaryId, tmp.entryId).subscribe({
        next: () => {
          let k = this.snackBar.open("Izmjena je sačuvana", "Ok").onAction().subscribe(res => {
            location.reload();
            k.unsubscribe();
          });
        },
  ...
  deniedWorkDiary() {
    this.workDiaryService.approveWorkDiary("DENIED", this.workDiaryId).subscribe({
      next: () => {
        let k = this.snackBar.open("Uspjesno!", "Ok").onAction().subscribe(() => {
          this._location.back();
          k.unsubscribe();
        })
      },
  ...
  approveWorkDiary() {
    this.workDiaryService.approveWorkDiary("ACCEPTED", this.workDiaryId).subscribe({
      next: () => {
        let k = this.snackBar.open("Uspjesno ste odobrili dnevnik rada!", "Ok").onAction().subscribe(() => {
          this._location.back();
          k.unsubscribe();
        })
      },
